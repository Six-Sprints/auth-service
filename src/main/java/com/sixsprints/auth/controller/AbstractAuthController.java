...
public abstract class AbstractAuthController<T extends AbstractAuthenticableEntity, DTO, DETAIL_DTO, L extends Authenticable, R extends ResetPasscode> {
  ...
  public ResponseEntity<RestResponse<AuthResponseDto<DETAIL_DTO>>> register(
      @RequestBody @Valid DTO dto) throws EntityAlreadyExistsException, EntityInvalidException {
  ...
  public ResponseEntity<RestResponse<String>> sendOtp(@RequestParam String authId)
    ...
    return RestUtil.successResponse(AuthMessageKeys.OTP_SENT_SUCCESSFULLY, HttpStatus.OK);
  }
  ...
  public ResponseEntity<RestResponse<String>> resetPassword(@RequestBody @Valid R resetDto)
    ...
    return RestUtil.successResponse(AuthMessageKeys.PASSWORD_RESET_SUCCESSFULLY, HttpStatus.OK);
  }
